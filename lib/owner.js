    ...
    switch (argv[2]) {
      ...
      case "rm":
        if (argv.length > 3) {
          ...
          return mapToRegistry(byUser, npm.config, function (er, uri, auth) {
            ...
            console.error(uri)
            registry.get(uri, { auth : auth }, function (er, d) {
              if (er) return cb(er)
      ...
      case "add":
        ...
        return mapToRegistry("-/users", npm.config, function (er, uri, auth) {
          ...
          registry.get(uri, { auth : auth }, function (er, list) {
            if (er) return cb()
...
function ls (pkg, cb) {
  ...
  mapToRegistry(pkg, npm.config, function (er, uri, auth) {
    ...
    registry.get(uri, { auth : auth }, function (er, data) {
      var msg = ""
      if (er) {
...
function mutate (pkg, user, mutation, cb) {
  if (user) {
    ...
    mapToRegistry(byUser, npm.config, function (er, uri, auth) {
      ...
      registry.get(uri, { auth : auth }, mutate_)
    })
  } else {
    mutate_(null, null)
  }
    ...
    mapToRegistry(pkg, npm.config, function (er, uri, auth) {
      ...
      registry.get(uri, { auth : auth }, function (er, data) {
        ...
        mapToRegistry(dataPath, npm.config, function (er, uri, auth) {
          ...
          registry.request("PUT", uri, { body : data, auth : auth }, function (er, data) {
